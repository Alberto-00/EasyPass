...
const main = (async () => {
	...
	server.on('request', async (req, res) => {
		...
		if(dgc === undefined) {
			...
			// 1. vaccination
			if(dcc.payload.v){
				validate = vaccination.validateVaccination(settings, dcc);
				certType = 'Vaccination';
			} 
			
			// 2. test
			if(dcc.payload.t){
				validate = test.validateTest(settings, dcc);
				certType = 'Test';
			...
			// 3. recovery
			if(dcc.payload.r){
				validate = recovery.validateRecovery(settings, dcc);
				certType = 'Recovery';
			} 
			...
			if(ADD_HOLDER_DETAILS) {
				...
				response = validate.message + ";" + surname + ";" + forename + ";" + dob + ";" + certType;
